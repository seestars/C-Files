...
int main(void)
{
    ...
    printf("Next enter the time in minutes ands seconds.\n");
    printf("Begin by entering the minutes.\n");
    scanf("%d", &min);
    printf("Now enter the seconds.\n");
    scanf("%d", &sec);

    time = S_PER_M * min + sec;     // 把时间和转换成秒
    ...
    printf("You ran %1.2f km (%1.2f miles) in %d min, %d sec.\n",
            distk, distm, min, sec);
    printf("Than pace corresponds to running a mile in %d min, ",
            mmin);
    printf("%d sec.\nYour average speed wa %1.2f mph.\n", msec,
            rate);
